...
.tour-shell { position: fixed; inset: 0; }
.tour-shell.is-loading iframe { opacity: 0.55; }
...
select, input, textarea, button, a { font: inherit; }
...
.cta-button, .ghost-button, .drawer-button, .quick-action {
    appearance: none;
    border: 0;
...
.cta-button:hover, .ghost-button:hover, .drawer-button:hover, .quick-action:hover {
    transform: translateY(-1px);
}

.cta-button, .ghost-button { border-radius: 999px; }
.drawer-button, .quick-action { border-radius: var(--radius-sm); }
...
.audio-box input[type="range"] { width: 112px; accent-color: var(--accent); }
...
.drawer-header, .drawer-content, .drawer-footer { padding: 22px 24px; }
...
.welcome-kicker, .quick-contact-label {
    color: var(--accent);
    font-size: 0.75rem;
    font-weight: 700;
...
.quick-contact-block { display: grid; gap: 10px; }
...
.quick-action svg { width: 17px; height: 17px; flex-shrink: 0; }

.quick-action.is-disabled, .drawer-button.is-disabled {
    opacity: 0.42;
    pointer-events: none;
}
...
.floating-field textarea { min-height: 136px; }
...
.email-providers { display: grid; gap: 8px; }
.email-providers.hidden { display: none; }
...
.gate-card h1 { margin: 0 0 10px; font-size: clamp(1.7rem, 4vw, 2.4rem); }
.gate-card p { margin: 0 0 24px; color: var(--muted); line-height: 1.65; }
...
.loading-tag { opacity: 0; transition: opacity 0.2s ease; }
.tour-shell.is-loading .loading-tag { opacity: 1; }
...
@media (max-width: 900px) {
    .hud-header { flex-wrap: wrap; padding: 10px 16px; }
    .header-brand img { max-width: 100px; max-height: 28px; }
    .brand-name { font-size: 0.88rem; }
    .property-title { font-size: 0.92rem; }
    .header-controls { gap: 8px; }
    .picker-wrap label { display: none; }
    .header-controls, .picker-wrap { min-width: 0; }
}
...
<body>
    <div class="tour-shell" id="tour-shell">
        <iframe id="tour-frame" title="Matterport tour"
            ...
            <div class="quick-contact-block">
                ...
                <div class="quick-actions">
                    <a id="call-link" class="quick-action" href="#">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.86 19.86 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6A19.86 19.86 0 0 1 2.12 4.18 2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.12.9.33 1.78.62 2.62a2 2 0 0 1-.45 2.11L8 9.91a16 16 0 0 0 6.09 6.09l1.46-1.28a2 2 0 0 1 2.11-.45c.84.29 1.72.5 2.62.62A2 2 0 0 1 22 16.92z"></path></svg>
                        <span>Call</span>
                    </a>
                    ...
                    <button type="button" id="email-link" class="quick-action">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="5" width="18" height="14" rx="2"></rect><path d="m4 7 8 6 8-6"></path></svg>
                        <span>Email</span>
                    </button>
                </div>
            </div>
...
const state = {
    activeIndex: 0, started: false, audioMuted: false,
    audioEnabled: false, audioSource: "", frameReady: false,
    headerVisible: true, headerAutoCollapsed: false, headerHideTimer: 0,
};
...
function buildMatterportUrl(model) {
    ...
    extraParams.forEach((value, key) => { if (key) { url.searchParams.set(key, value); } });
    return url.toString();
}

function getActiveModel() { return CONFIG.models[state.activeIndex]; }
...
function scheduleHeaderAutoHide() {
    if (!state.started || state.headerAutoCollapsed) { return; }
    window.clearTimeout(state.headerHideTimer);
...
function updateQuickActions() {
    ...
    if (phone) {
        ...
        el.callLink.href = "#"; el.callLink.classList.add("is-disabled");
        el.smsLink.href = "#"; el.smsLink.classList.add("is-disabled");
    }
...
function renderPropertyPicker() {
    ...
    CONFIG.models.forEach((model, index) => {
        ...
        option.textContent = model.location ? model.name + " | " + model.location : model.name;
...
function renderBrand() {
    ...
    if (showLogo) {
        el.hudLogo.src = CONFIG.logoDataUrl; el.hudLogo.hidden = false;
        el.gateLogo.src = CONFIG.logoDataUrl; el.gateLogo.hidden = false;
    } else {
        el.hudLogo.hidden = true; el.gateLogo.hidden = true;
...
function renderBuildStamp() {
    if (!CONFIG.buildInfo || !CONFIG.buildInfo.version) { el.buildStamp.textContent = ""; return; }
...
async function startExperience(muted) {
    if (state.started) { el.gatekeeper.remove(); return; }
    state.started = true;
    state.audioMuted = Boolean(muted);
    el.audio.muted = state.audioMuted;
    updateMuteLabel();
    await syncAudioForActiveModel(!muted);
    if (state.audioEnabled && !muted) { el.gateStatus.textContent = "Ambient audio started."; }
    el.gatekeeper.remove();
    if (state.frameReady) { scheduleHeaderAutoHide(); }
}
...
function boot() {
    ...
    if (!CONFIG.logoDataUrl) { el.brandLockup.style.gap = "0"; }
...
el.hudToggle.addEventListener("click", () => {
    ...
    if (nextVisible) { window.clearTimeout(state.headerHideTimer); }
});

el.contactToggle.addEventListener("click", () => setDrawerOpen(true));
el.closeDrawer.addEventListener("click", () => { setDrawerOpen(false); });
...
el.volumeRange.addEventListener("input", () => { el.audio.volume = Number(el.volumeRange.value); });

el.muteToggle.addEventListener("click", async () => {
    if (!getActiveMusicSource()) { return; }
    state.audioMuted = !state.audioMuted;
    el.audio.muted = state.audioMuted;
    updateMuteLabel();
    if (!state.audioMuted) {
        try { await syncAudioForActiveModel(true); }
        catch (error) { state.audioMuted = true; el.audio.muted = true; updateMuteLabel(); }
    }
});
